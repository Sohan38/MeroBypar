...
export function useInventoryFormSteps(
                    ...
                    <div className="p-6 md:p-8 space-y-6">
                        ...
                        {quantity === 0 && (
                            <button
                                type="button"
                                onClick={() => {
                                    const isBulkActive = Boolean(
                                        form.getValues('isPackPricingEnabled') ||
                                        (Number(form.getValues('packSize')) > 0)
                                    );
                                    const targetStepId = (isBulkActive || hasExpiry) ? 'inventory' : 'pricing-stock';
                                    const targetIndex = steps.findIndex((s) => s.id === targetStepId);
                                    if (targetIndex >= 0) {
                                        setActiveStep(targetIndex);
                                    }
                                }}
                                className="w-full text-left flex items-center justify-between gap-3 p-4 rounded-2xl border border-amber-200 bg-amber-50/50 hover:bg-amber-100/60 dark:border-amber-900/40 dark:bg-amber-950/20 dark:hover:bg-amber-950/40 transition-colors group cursor-pointer"
                            >
                                <div className="flex items-start gap-3 text-amber-800 dark:text-amber-300 text-xs min-w-0">
                                ...
                                <span className="shrink-0 text-xs font-semibold text-amber-900 dark:text-amber-200 underline underline-offset-2 flex items-center gap-1">
                                    Set stock
    ...
    }, [steps, stepErrorsWithUI, form, setActiveStep, quantity, hasExpiry]);
